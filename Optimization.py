...
class Optimization(para,options):
    def __init__(self,fh,rho,vp):
        super().__init__()
        self.fh=fh
        self.data=np.hstack((rho.flatten(),vp.flatten()))
    def optimization(self):
        ...
        if not os.path.exists('./imodel_file'):
            os.makedirs('./imodel_file')
        else:
            shutil.rmtree('./imodel_file')
            os.makedirs('./imodel_file')
...
#        alpha=0
        ...
        f,g=self.fh(x[:int(len(x)/2)].reshape((para.xl+2*para.CPML,-1)),\
                    x[int(len(x)/2):].reshape((para.xl+2*para.CPML,-1)))
...
#                alpha0=min(1,2*(f-f_old)/(gtd))
            ...
            x=x+alpha*d
            x=self.__counts(x)
            np.save('./imodel_file/%d_imodel.npy'%(iter_),x)
            info.append([iter_+1,fevals,alpha,f,np.linalg.norm(g,2)])
    ...
    def __WolfeLineSearch(self,x,t,d,f,g,gtd,c1,c2,LS_interp,maxLS,debug,doPlot,fh):
        ...
        f_new,g_new=fh(x_new[:int(len(x)/2)].reshape((para.xl+2*para.CPML,-1)),\
                       x_new[int(len(x)/2):].reshape((para.xl+2*para.CPML,-1)))
        lsiter=1
